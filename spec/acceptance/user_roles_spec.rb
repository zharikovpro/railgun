RSpec.resource 'UserRoles', issues: ['railgun#168'] do
  ...
  post '/api/v1/user_roles' do
    ...
    example_request 'Create user role' do
      ...
      expect(JSON.parse(response_body)['user_role']).to eq('editor')
      expect(User.find(user.id).roles).to match('editor')
    end
  end
end
